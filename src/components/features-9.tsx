            ...
            <div className="mx-auto max-w-5xl border border-border rounded-3xl overflow-hidden bg-card/40 backdrop-blur-md grid md:grid-cols-2">
                    ...
                    <div aria-hidden className="flex flex-col gap-6 mt-8">
                        <div className="text-left">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-muted-foreground text-[10px] uppercase font-bold tracking-wider">Member Exporter</span>
                            </div>
                            <div className="rounded-2xl bg-card border border-border mt-1.5 w-4/5 p-4 text-xs text-foreground shadow-sm">
                              Hello, we need an urgent attestation validation for our customs clearance at Nhava Sheva port.
                            </div>
                        </div>

                        <div className="text-right">
                            <div className="rounded-2xl mb-1 ml-auto w-4/5 bg-primary p-4 text-xs text-primary-foreground shadow-md text-left">
                              Our digital e-platform verification is complete. The Certificate of Origin has been cryptographically signed and routed.
                            </div>
                            <span className="text-muted-foreground block text-[10px] uppercase font-bold tracking-wider mr-2">Chamber Agent</span>
                        </div>
                    </div>
                </div>

                <div className="col-span-full border-y border-border p-8 bg-card/60">
                    <p className="text-center text-3xl font-heading font-black text-foreground md:text-5xl">99.99% E-Platform Uptime</p>
                </div>
                ...
                <div className="relative col-span-full text-left">
                    <div className="absolute z-10 max-w-lg px-8 pr-12 pt-8 md:px-12 md:pt-12">
                        ...
                        <p className="my-6 text-xl font-heading font-black text-foreground">
                          Real-time processing logs. <span className="text-muted-foreground font-normal">Monitor application volumes and attestation speed.</span>
                        </p>
                    </div>
                    <MonitoringChart />
                </div>
...
const Map = () => {
    ...
    const points = [
      { x: 20, y: 15 }, { x: 25, y: 18 }, { x: 22, y: 22 },
      { x: 30, y: 25 }, { x: 35, y: 22 }, { x: 32, y: 30 },
      { x: 40, y: 28 }, { x: 45, y: 24 }, { x: 42, y: 35 },
      { x: 50, y: 32 }, { x: 55, y: 28 }, { x: 52, y: 38 },
      { x: 60, y: 35 }, { x: 65, y: 32 }, { x: 62, y: 40 },
      { x: 70, y: 38 }, { x: 75, y: 35 }, { x: 72, y: 45 },
      { x: 80, y: 42 }, { x: 85, y: 38 }, { x: 82, y: 48 },
    ]
    return (
        <svg viewBox={viewBox} className="w-full h-auto text-primary opacity-60">
            ...
            {points.map((point, index) => (
                <circle 
                  ...
                  cy={point.y} 
                  r={index % 3 === 0 ? 1.2 : 0.8} 
                  className={index % 3 === 0 ? "fill-primary" : "fill-muted-foreground/40"} 
                />
            ))}
        </svg>
    )
...
const MonitoringChart = () => {
    return (
        <ChartContainer className="h-96 w-full aspect-auto pt-48" config={chartConfig}>
            <AreaChart
                accessibilityLayer
                data={chartData}
                margin={{
                ...
                <defs>
                    ...
                    <linearGradient id="fillMobile" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <stop offset="100%" stopColor="var(--color-muted-foreground)" stopOpacity={0.0} />
                    </linearGradient>
                </defs>
                <CartesianGrid vertical={false} strokeDasharray="3 3" opacity={0.1} />
                <ChartTooltip active cursor={false} content={<ChartTooltipContent className="bg-card border-border" />} />
                <Area strokeWidth={2} dataKey="mobile" type="monotone" fill="url(#fillMobile)" stroke="var(--color-muted-foreground)" stackId="a" />
                <Area strokeWidth={2.5} dataKey="desktop" type="monotone" fill="url(#fillDesktop)" stroke="var(--color-primary)" stackId="b" />
            </AreaChart>
        </ChartContainer>
    )
}
