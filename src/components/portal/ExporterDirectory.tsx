...
const EXPORTERS_DATA: Exporter[] = [
  ...
  },
  {
    ...
    products: [
      "TradeLog SaaS",
      "AI Supply Chain Optimizers",
      "Chamber Integration APIs",
    ],
    tier: "Premium",
    verified: false,
    country: "Singapore",
...
export default function ExporterDirectory() {
  ...
  const [selectedExporter, setSelectedExporter] =
    React.useState<Exporter | null>(null);
  const [rfqMessage, setRfqMessage] = React.useState("");

  // Filtered Exporters
  const filteredExporters = EXPORTERS_DATA.filter((exp) => {
    const matchesCat =
      forumCategory === "all" || exp.category === forumCategory;
    const matchesSearch =
      exp.name.toLowerCase().includes(forumSearch.toLowerCase()) ||
      exp.products.some((p) =>
        p.toLowerCase().includes(forumSearch.toLowerCase()),
      ) ||
  ...
  const handleRfqSubmit = (e: React.FormEvent) => {
    ...
    toast.success(
      `Request for Quote successfully routed to ${selectedExporter?.name}! You will receive their proposal shortly via registered email.`,
    );
    setRfqMessage("");
      ...
      <div className="mx-auto max-w-[1280px] px-6 md:px-12">
        ...
        >
          ...
          <p className="font-body text-sm md:text-base text-muted-foreground mt-4 max-w-2xl mx-auto">
            Securely match, contact, and negotiate trade deals with verified
            corporate members certified by the AECCI regulatory board.
          </p>
        </motion.div>
              ...
              >
                <Card className="h-full border-border bg-card/75 backdrop-blur-xl shadow-xl flex flex-col justify-between hover:border-primary/30 hover:shadow-2xl transition-all duration-300">
                  <CardHeader className="p-6 pb-2 text-left">
                    ...
                    <h3 className="font-heading font-black text-base text-foreground mt-4">
                      {exp.name}
                    </h3>
                  ...
                  <CardContent className="p-6 pt-2 pb-4 text-left">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">
                      Primary Export Portfolio
                    </p>
                    <div className="flex flex-wrap gap-1.5">
                      {exp.products.map((prod, idx) => (
                        <span
                          key={idx}
                          className="bg-muted/40 border border-border/80 px-2 py-0.5 rounded text-[10px] text-foreground font-semibold"
                        >
                          {prod}
                        </span>
                      ))}
                  ...
                  <CardFooter className="p-6 pt-0 flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        toast.info(
                          `Direct chat pipeline established with ${exp.name}`,
                        )
                      }
            ...
            {filteredExporters.length === 0 && (
              <div className="col-span-full border border-dashed border-border p-16 rounded-xl text-center text-muted-foreground">
                ...
                <p className="font-heading font-bold text-base">
                  No Exporters Match Your Criteria
                </p>
                <p className="text-xs max-w-sm mx-auto mt-2">
                  Adjust your category filter or clean your search text queries
                  to find associated exporters.
                </p>
              </div>
            )}
          </motion.div>
            ...
            <Card className="w-full max-w-[480px] border-border bg-card shadow-2xl p-6 text-left relative animate-in zoom-in-95 duration-300">
              ...
              <CardHeader className="p-0 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <span className="px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-bold rounded border border-primary/20 uppercase tracking-widest">
                    RFQ
                  </span>
                </div>
                <CardDescription className="text-xs text-muted-foreground mt-1">
                  You are requesting commercial quotations directly from{" "}
              ...
              <form onSubmit={handleRfqSubmit} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    Detailed Shipping & Volume Specifications
                  </label>
                  <textarea
                    id="rfq-msg"
                    rows={4}
