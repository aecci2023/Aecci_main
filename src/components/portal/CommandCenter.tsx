...
import { Database, Target, Handshake, Compass, Terminal, Calendar, ChevronRight } from "lucide-react";
...
type CountryData = {
  name: string; code: string; flag: string; overview: string; opportunity: string;
...
const COUNTRIES: Record<string, CountryData> = {
  KENYA: {
    ...
    overview: "Fastest-growing tech & commerce hub in East Africa. Ideal entry point for agricultural machinery, FMCG, and construction materials.",
  ...
  NETHERLANDS: {
    ...
    overview: "Primary gateway to the European Union market. Massive demand for biological goods, medical equipment, and organic chemicals.",
  ...
  SINGAPORE: {
    ...
    overview: "Financial capital of Southeast Asia. Hub for advanced technology products, fintech solutions, and premium gourmet food products.",
  ...
  MEXICO: {
    ...
    overview: "Key manufacturing hub bordering North America. Rapidly growing demand for automotive components and heavy industrial raw materials.",
  ...
  GHANA: {
    ...
    overview: "Extremely stable economy with strong trade agreements. Primary demand includes building materials, agricultural inputs, and pharmaceuticals.",
...
const DEMAND_CONFIG = {
  ...
  HIGH: { label: "High", class: "bg-chart-4/10 text-chart-4 border-chart-4/20" },
  STEADY: { label: "Steady", class: "bg-primary/10 text-primary border-primary/20" },
};
...
export default function CommandCenter() {
  ...
  return (
    <section id="command-center" className="bg-background py-16 md:py-24 border-b border-border/60 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-[1280px] px-6 md:px-12 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <p className="text-muted-foreground text-base leading-relaxed">
            Select a target market to unlock live intelligence, sector demands, distributor vacancies, and upcoming deal room schedules.
          </p>
        </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={cn(
                      ...
                      country.demandLevel === "CRITICAL" ? "text-destructive" : country.demandLevel === "HIGH" ? "text-chart-4" : "text-primary",
                    )}>
                      {country.volumeGrowth}
                    </span>
                    <span className={cn(
                      "size-1.5 rounded-full",
                      country.demandLevel === "CRITICAL" ? "bg-destructive" : country.demandLevel === "HIGH" ? "bg-chart-4" : "bg-primary",
                    )} />
                  </div>
                </button>
              );
          ...
          <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Card className="md:col-span-2 rounded-2xl border-border/60 bg-card/50 backdrop-blur-sm p-5 relative overflow-hidden">
              ...
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border/50">
                ...
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">01 / Market Intelligence</span>
              </div>
              <AnimatePresence mode="wait">
                <motion.div key={current.name} variants={fade} initial="hidden" animate="show" exit="exit">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-3xl">{current.flag}</span>
                    <div>
                      <h3 className="text-lg font-bold text-foreground">{current.name} Trade Corridor</h3>
                      <div className="flex items-center gap-2 mt-0.5">
                        <Badge className={cn("text-[9px] font-bold border", demandCfg.class)}>{demandCfg.label} Demand</Badge>
                        <span className="text-[10px] text-primary font-black font-mono">{current.volumeGrowth} growth</span>
                      </div>
                    </div>
                  </div>
                  <p className="text-foreground/80 text-sm leading-relaxed font-light">{current.overview}</p>
                </motion.div>
              </AnimatePresence>
            </Card>
            ...
            <Card className="rounded-2xl border-border/60 bg-card/50 backdrop-blur-sm p-5">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border/50">
                ...
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">02 / Buyer Vacancies</span>
              </div>
              <AnimatePresence mode="wait">
                <motion.div key={current.name} variants={fade} initial="hidden" animate="show" exit="exit" className="space-y-3">
                  <p className="text-foreground/85 text-xs leading-relaxed">{current.opportunity}</p>
                  <div className="flex gap-2 flex-wrap">
                    <Badge variant="outline" className="text-[9px] font-mono bg-muted/50 border-border">Target: Exporters</Badge>
                    <Badge className="bg-chart-4/10 text-chart-4 border border-chart-4/20 text-[9px] font-mono">Slots Available</Badge>
                  </div>
                </motion.div>
              </AnimatePresence>
            </Card>
            ...
            <Card className="rounded-2xl border-border/60 bg-card/50 backdrop-blur-sm p-5">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border/50">
                ...
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">03 / Sector Demand</span>
              </div>
              <AnimatePresence mode="wait">
                <motion.div key={current.name} variants={fade} initial="hidden" animate="show" exit="exit" className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-muted-foreground font-mono">INDICATOR:</span>
                    <Badge className={cn("font-mono font-bold text-[9px] tracking-widest uppercase border", demandCfg.class)}>
                      {current.demandLevel}
                    </Badge>
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground font-mono mb-1">DEMAND SECTORS:</p>
                    <p className="text-sm font-bold text-foreground">{current.demand}</p>
                  </div>
                  <div className="flex items-center gap-4 pt-2 border-t border-border/40 text-[10px] font-mono">
                    ...
                    <span className="text-muted-foreground">PEAK: <strong className="text-foreground">{current.topSector}</strong></span>
                  </div>
                </motion.div>
              </AnimatePresence>
            </Card>
            ...
            <Card className="rounded-2xl border-border/60 bg-card/50 backdrop-blur-sm p-5">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border/50">
                ...
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">04 / Access Partner</span>
              </div>
              <AnimatePresence mode="wait">
                <motion.div key={current.name} variants={fade} initial="hidden" animate="show" exit="exit" className="space-y-3">
                  ...
                  <p className="text-[10px] text-muted-foreground font-mono leading-relaxed">
                    Legal support, localized compliance validation, and verified buyer matchmaking.
                  </p>
                </motion.div>
              </AnimatePresence>
            </Card>
            ...
            <Card className="rounded-2xl border-border/60 bg-card/50 backdrop-blur-sm p-5">
              <div className="flex items-center gap-2 mb-3 pb-3 border-b border-border/50">
                ...
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">05 / Deal Room Slot</span>
              </div>
              <AnimatePresence mode="wait">
                <motion.div key={current.name} variants={fade} initial="hidden" animate="show" exit="exit" className="space-y-3">
                  <div>
                    <p className="text-[10px] text-muted-foreground font-mono mb-1">UPCOMING SESSION:</p>
                  ...
                  <div className="flex items-center justify-between text-xs pt-2 border-t border-border/40 font-mono">
                    ...
                    <span className="text-primary font-bold text-[10px]">MATCHMAKING ACTIVE</span>
                  </div>
                  <Button asChild size="sm" className="w-full rounded-lg text-xs font-bold bg-primary/10 text-primary border border-primary/20 hover:bg-primary hover:text-primary-foreground transition-all duration-200">
                    <a href="#pricing">Book This Session <ChevronRight className="size-3.5 ml-1" /></a>
                  </Button>
                </motion.div>
              </AnimatePresence>
            </Card>
