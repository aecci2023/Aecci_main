...
const REGIONS: Record<string, { partners: Partner[]; color: string; label: string }> = {
  AFRICA: {
    label: "Africa",
    color: "text-chart-4",
    partners: [
...
const cardVariants = {
  ...
  show: (i: number) => ({ opacity: 1, scale: 1, transition: { duration: 0.4, delay: i * 0.06, ease: [0.16, 1, 0.3, 1] as [number, number, number, number] } }),
      ...
      <div className="mx-auto max-w-[1280px] px-6 md:px-12 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="text-muted-foreground text-base leading-relaxed">
            We partner with leading sovereign trade authorities, ports, and chambers to validate markets and connect you with verified local buyers.
          </p>
        </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span className="text-[11px] font-bold font-mono uppercase tracking-wide">{p.location}</span>
                      </div>
                      <Badge variant="outline" className="text-[9px] font-mono text-muted-foreground shrink-0 border-border bg-muted/50">
                        Verified
                      </Badge>
                    </div>

                    <h4 className={cn("text-sm font-bold text-foreground group-hover:transition-colors leading-snug", `group-hover:${color}`)}>
                      {p.name}
                    </h4>
                    ...
                    <div className="space-y-1.5 pt-3 border-t border-border/50 text-xs text-muted-foreground">
                      <p className="flex items-start gap-2">
                        ...
                        <span>Role: <strong className="text-foreground/80">{p.role}</strong></span>
                      </p>
                      <p className="flex items-start gap-2">
                        <ShieldCheck className="size-3.5 shrink-0 mt-0.5 text-muted-foreground/70" />
                        <span>Auth: <strong className="text-foreground/80">{p.certification}</strong></span>
                      </p>
                    </div>
                  </div>
