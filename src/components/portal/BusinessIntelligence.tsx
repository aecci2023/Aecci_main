...
const CLEARANCE_STATS = [
  {
    label: "Avg e-CO Attestation",
    value: "4m 12s",
    sub: "99.87% Accuracy rate",
    color: "text-primary",
  },
  {
    label: "Daily Requests Filed",
    value: "1,842",
    sub: "Secure API routing active",
    color: "text-chart-2",
  },
  {
    label: "Port Clearance Uplift",
    value: "+65%",
    sub: "Faster transit times",
    color: "text-chart-3",
  },
];
      ...
      <div className="mx-auto max-w-[1280px] px-6 md:px-12 relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-14">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-foreground tracking-tight leading-[1.1] mb-4">
            Live Business <span className="text-primary">Intelligence</span>
          </h2>
          <p className="text-muted-foreground text-base leading-relaxed">
            Real-time market insights, attestation volumes, and trending product
            segments from our digital shipping registries.
          </p>
        </div>
          ...
          >
            <Card className="h-full rounded-2xl border-border/60 bg-card/50 backdrop-blur-sm p-6 flex flex-col shadow-sm">
              ...
              <p className="text-[11px] text-muted-foreground mb-5 font-light">
                Relative volume indexed by Certificate of Origin submissions
                this quarter.
              </p>
              <div className="flex-1 min-h-[220px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={SECTOR_DATA}
                    margin={{ left: -25, right: 8, bottom: 0, top: 8 }}
                  >
                    <CartesianGrid
                      strokeDasharray="3 3"
                      vertical={false}
              ...
              <div className="flex-1 space-y-0 divide-y divide-border/40">
                {CLEARANCE_STATS.map((s) => (
                  ...
                  >
                    ...
                    <p
                      className={`text-3xl font-black tracking-tight leading-none ${s.color}`}
                    >
                      {s.value}
                    </p>
                    <p
                      className={`text-[10px] font-medium ${s.color} opacity-80`}
                    >
                      {s.sub}
                    </p>
                  </div>
