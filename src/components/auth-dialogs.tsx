...
export function AuthDialog({
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setIsOpen(false);
      toast.success(
        `Welcome back! Successfully logged in as ${loginEmail.split("@")[0]}`,
      );
      setLoginEmail("");
      setLoginPassword("");
    }, 1500);
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setIsOpen(false);
      toast.success(
        `Registration request submitted! Our membership team will contact ${regCompany} within 24 hours.`,
        {
          duration: 5000,
        },
      );
      ...
      <DialogContent className="sm:max-w-[450px] border-border bg-card/95 backdrop-blur-xl text-card-foreground shadow-2xl p-6">
        <DialogHeader className="space-y-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="px-2 py-0.5 text-[10px] font-semibold bg-primary/10 text-primary rounded border border-primary/20 uppercase tracking-widest">
              Secure Access
            </span>
          </div>
        ...
        >
          <TabsList className="grid w-full grid-cols-2 bg-muted/50 p-1 rounded-lg">
            <TabsTrigger
              value="login"
              className="rounded-md py-2 text-xs font-semibold data-[state=active]:bg-background data-[state=active]:text-foreground transition-all"
            >
              Member Sign In
            </TabsTrigger>
            <TabsTrigger
          ...
          >
            <form onSubmit={handleLogin} className="space-y-4">
              <div className="space-y-2">
                ...
                >
                  <Envelope className="size-3.5 text-primary" /> Registered
                  Email Address
                </Label>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      toast.info(
                        "Password reset instructions sent to registered administrator email.",
                      );
                    }}
              ...
              <Button
                ...
                className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-10 font-bold transition-all mt-6 shadow-lg shadow-primary/10"
              >
            ...
            <form onSubmit={handleRegister} className="space-y-3">
              <div className="space-y-1.5">
                <Label
                  ...
                  className="text-xs font-medium text-muted-foreground flex items-center gap-1.5"
                >
                  <User className="size-3.5 text-primary" /> Contact
                  Representative
                </Label>
                <Input
                  id="reg-name"
              ...
              <div className="space-y-1.5">
                <Label
                  ...
                  className="text-xs font-medium text-muted-foreground flex items-center gap-1.5"
                >
                  <Envelope className="size-3.5 text-primary" /> Professional
                  Email
                </Label>
                <Input
                  id="reg-email"
              ...
              <div className="space-y-1.5">
                <Label
                  ...
                  className="text-xs font-medium text-muted-foreground flex items-center gap-1.5"
                >
                  <IdentificationCard className="size-3.5 text-primary" />{" "}
              ...
              <div className="space-y-1.5">
                <Label
                  ...
                  className="text-xs font-medium text-muted-foreground flex items-center gap-1.5"
                >
                  <ShieldCheck className="size-3.5 text-primary" /> Membership
                  Designation
                </Label>
                <select
                  id="reg-type"
                ...
                >
                  <option value="exporter" className="bg-card text-foreground">
                    Global Exporter Member
                  </option>
                  <option value="importer" className="bg-card text-foreground">
                    Strategic Importer Partner
                  </option>
                  <option
                    value="chamber-alliance"
                    className="bg-card text-foreground"
                  >
                    Chamber Alliance Associate
                  </option>
                  <option value="startup" className="bg-card text-foreground">
                    Youth Wing Startup
                  </option>
                </select>
              </div>
              ...
              <div className="text-[10px] text-muted-foreground mt-2 leading-relaxed bg-muted/30 p-2.5 rounded border border-border/50 flex items-start gap-1.5">
                ...
                <span>
                  By submitting, you request formal enrollment in the Asian
                  Exporters' Chamber of Commerce and Industry. A verified trade
                  officer will evaluate your corporate filings within 1 working
                  day.
                </span>
              </div>
              ...
              <Button
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-10 font-bold transition-all mt-4 shadow-lg shadow-primary/10"
              >
                {isSubmitting
                  ? "Submitting Corporate Portfolio..."
                  : "Request Access Key"}
              </Button>
            </form>
          </TabsContent>
