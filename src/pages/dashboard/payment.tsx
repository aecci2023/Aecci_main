...
export default function PaymentPage() {
  ...
  const { data: historyData, isLoading: isHistoryLoading } = useGetSubscriptionHistoryQuery(undefined, {
    skip: !currentUser?.id,
  });

  const [createOrder, { isLoading: isCreating }] = useCreateSubscriptionOrderMutation();
  const [verifyPayment, { isLoading: isVerifying }] = useVerifySubscriptionPaymentMutation();
        ...
        handler: async function (response: any) {
          try {
            ...
            if (verifyRes.success) {
              ...
              const updatedUser = { 
                ...currentUser, 
                kycStatus: dbUser.kycStatus === 'approved' ? 'active' : dbUser.kycStatus, 
                planName: planId,
                planActive: true 
              };
      ...
      <Main fluid className="space-y-10 py-6 max-w-7xl mx-auto">
        <div className="text-center space-y-3">
          ...
          <p className="text-muted-foreground text-base max-w-2xl mx-auto">
            Unlock guaranteed 1-on-1 matches with verified buyers, customs experts, and international trading partners.
          </p>
        </div>
          ...
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-4 max-w-2xl mx-auto flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-emerald-700/80 dark:text-emerald-400/80">
                Currently subscribed to the <strong className="uppercase">{dbUser.planName} Plan</strong> with <strong>{dbUser.slotsRemaining} / {dbUser.slotsTotal}</strong> session slots remaining.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {PLANS.map((plan) => {
            const isCurrent = dbUser?.planName === plan.id && dbUser?.planActive;
            return (
              <Card 
                key={plan.id} 
                className={`flex flex-col relative border shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-1 ${
                  plan.popular ? "border-emerald-500 ring-2 ring-emerald-500/20" : "border-border"
                }`}
              >
                {plan.popular && (
                ...
                <CardHeader className="pb-4">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {isCurrent && (
                      <Badge variant="outline" className="bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400 border-emerald-500/30">
                        Current
                      </Badge>
                    )}
                  </div>
        ...
        <div className="flex flex-col items-center justify-center gap-2 max-w-sm mx-auto text-center border p-4 rounded-xl bg-muted/20">
          ...
          <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
            <ShieldCheck className="size-3.5 text-emerald-500" /> Secure checkout processed via Razorpay gateway.
          </div>
        </div>
        ...
        <div className="pt-10 border-t">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h2 className="text-2xl font-bold tracking-tight">Billing & Invoice History</h2>
              <p className="text-muted-foreground text-xs">Receipts and PDF invoices of your platform plan purchases.</p>
            </div>
          </div>

          <div className="bg-background border rounded-lg overflow-hidden">
            {isHistoryLoading ? (
              <div className="p-8 text-center text-muted-foreground animate-pulse text-sm">Loading billing history...</div>
            ) : history.length === 0 ? (
                  ...
                  {history.map((purchase: any) => (
                    <TableRow key={purchase.id}>
                      ...
                      <TableCell className="text-right">
                        {purchase.invoiceUrl ? (
                          <Button asChild size="sm" variant="ghost" className="h-8 text-xs text-primary hover:underline">
                            <a href={purchase.invoiceUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5">
