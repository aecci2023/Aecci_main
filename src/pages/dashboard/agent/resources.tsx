...
export default function AgentResourcesPage() {
  return (
    ...
    >
      ...
      {/* Main + right sidebar aligned from category cards down */}
      <div className="grid grid-cols-1 items-start gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
        <div className="min-w-0 space-y-5">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <button
              type="button"
              className="flex h-11 items-center justify-between gap-3 rounded-xl border border-[#E4E7EC] bg-white px-3.5 text-[13px] font-medium text-[#344054] shadow-[0_1px_2px_rgba(16,24,40,0.04)] sm:min-w-[180px]"
            >
              All Categories
          ...
          {/* Category / stats cards — same row level as right sidebar */}
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-3 2xl:grid-cols-5">
            {CATEGORIES.map((cat) => (
              <button
                key={cat.title}
                type="button"
                className="rounded-2xl border border-[#E4E7EC] bg-white p-4 text-left shadow-[0_1px_2px_rgba(16,24,40,0.05)] transition hover:border-[#D0D5DD] hover:shadow-sm"
              >
                <span
