...
export default function PartnerBriefPage() {
  ...
  const { data: partnerData, isLoading: isPartnerLoading } =
    useGetMarketplacePartnerDetailQuery(partnerUserId, {
      skip: !partnerUserId,
    });
  ...
  const handleBookingSubmit = async (e: React.FormEvent) => {
    ...
    const dateDayName = new Intl.DateTimeFormat("en-US", dayOptions).format(
      selectedDateObj,
    );
    ...
    return (
      <Main fluid className="max-w-4xl mx-auto py-12 text-center space-y-4">
        ...
        <p className="text-muted-foreground text-sm">
          The selected partner profile does not exist or has not been approved.
        </p>
      ...
      <Main fluid className="space-y-6 py-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Review trade specialties and check calendar availability before
            requesting a Deal Room booking.
          </p>
        </div>
        ...
        <Card className="border-amber-500/20 bg-amber-500/5">
          <CardContent className="pt-4 flex gap-3 items-start">
            ...
            <div className="text-xs text-muted-foreground leading-relaxed">
              <strong>Collaboration Confidentiality:</strong> All direct contact
              details (emails, phone numbers, and social links) are strictly
              protected by the chamber ecosystem until a formal session is
              approved by AECCI.
            </div>
          </CardContent>
        </Card>
              ...
              <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pb-4 border-b">
                ...
                <div className="space-y-1">
                  ...
                  <CardDescription className="flex items-center gap-1 mt-1 text-sm font-medium">
                    ...
                    {partner.organization || "Chamber Delegate"}
                  </CardDescription>
                </div>
              </CardHeader>
              <CardContent className="space-y-6 pt-6 text-sm leading-relaxed">
                <div>
                  ...
                  <p className="text-muted-foreground whitespace-pre-line">
                    {partner.bio || "No professional biography provided yet."}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t pt-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {partner.expertiseCountries?.map(
                ...
                <div className="border-t pt-4 grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="font-semibold text-foreground text-sm flex items-center gap-1.5 mt-0.5">
                      ...
                      {userDetails.languagesSpoken?.join(", ") || "English"}
                    </span>
                  </div>
                  <div>
                    <span className="text-xs text-muted-foreground block">
            ...
            <Card className="shadow-sm border-border bg-muted/10">
              <CardHeader className="pb-3 border-b">
                <CardTitle className="text-lg flex items-center gap-2">
                  <Calendar className="size-5 text-primary" /> Weekly Slot
                  Calendar
                </CardTitle>
                <CardDescription className="text-xs">
                  Select an available day to draft your Deal Room booking
                  request.
                </CardDescription>
              </CardHeader>
                  ...
                  return (
                    ...
                    >
                      ...
                      {isAvailable ? (
                        ...
                        <span className="text-[10px] text-muted-foreground pr-2 font-mono">
                          CLOSED
                        </span>
                      )}
                    </div>
        ...
        <Dialog open={isBookingOpen} onOpenChange={setIsBookingOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              ...
              <DialogDescription>
                Provide detailed objectives for your 1-on-1 meeting. Your
                request goes to the Admin for approval.
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleBookingSubmit} className="space-y-4 py-4">
              ...
              <div className="space-y-1.5">
                ...
                <span className="text-[10px] text-amber-600 block">
                  Must select a date corresponding to a{" "}
              ...
              <div className="space-y-1.5">
                <Label
                  ...
                  className="text-xs font-semibold text-muted-foreground uppercase"
                >
                  Specific Questions for the Partner (Optional)
                </Label>
                <Textarea
                  id="questions"
                  value={questions}
          ...
          <DialogContent className="max-w-sm text-center py-6">
            <DialogHeader className="items-center">
              ...
              <DialogDescription className="text-xs">
                You must purchase an active subscription plan to schedule
                sessions in the Deal Room.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4 text-xs text-muted-foreground leading-relaxed">
              Your account currently has{" "}
              <strong className="text-foreground">
                0 active Deal Room credits
              </strong>
              . explorer, Growth, and Market Entry plans provide matching
              session slot credits.
            </div>
