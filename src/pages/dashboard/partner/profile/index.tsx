...
export default function PartnerProfilePage() {
  const { data: profileData } = useGetMyPartnerProfileQuery();
  const profile = profileData?.data;
  const userStr = typeof window !== "undefined" ? localStorage.getItem("user") : null;
  const localUser = userStr ? JSON.parse(userStr) : null;
  const user = profile?.user || localUser;

  // Fallbacks to match mockup if user data isn't filled yet
  const fullName = user?.fullName || "Michael Anderson";
  const email = user?.email || "michael.anderson@aecci.org";
  const mobile = (user?.countryCode && user?.mobileNumber) ? `${user?.countryCode} ${user?.mobileNumber}` : (user?.mobileNumber || "+1 (202) 555 0184");
  const designation = user?.professionalTitle || "Senior Trade Advisor";
  const company = profile?.organization || "Global Trade Solutions LLC";
    ...
    <Main fluid className="bg-[#F8FAFC] min-h-screen p-4 sm:p-8 space-y-6 text-left">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
          ...
          <div className="lg:col-span-4 flex flex-col items-center text-center lg:border-r lg:border-slate-100 lg:pr-8">
            <div className="relative inline-block">
              <img
                src={user?.profilePicture || "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=250&h=250&q=80"}
                alt={fullName}
          ...
          {/* Right Column: Personal Information Key-Value Details */}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3.5 text-xs sm:text-sm pt-1">
              <div className="flex items-start">
                <span className="text-slate-400 font-semibold w-32 shrink-0 pt-0.5">Business Type</span>
                <span className="text-slate-900 font-bold">Trade Advisory & Consulting</span>
              </div>

              <div className="flex items-start">
                <span className="text-slate-400 font-semibold w-32 shrink-0 pt-0.5">Area of Expertise</span>
                <div className="space-y-0.5">
                  <p className="text-slate-900 font-bold">International Trade</p>
                  <p className="text-xs text-slate-400 font-medium">Market Entry, Trade Strategy, Legal Compliance</p>
                </div>
              </div>

              <div className="flex items-start">
                <span className="text-slate-400 font-semibold w-32 shrink-0 pt-0.5">Experience</span>
                <span className="text-slate-900 font-bold">10+ Years</span>
              </div>

              <div className="flex items-start">
                <span className="text-slate-400 font-semibold w-32 shrink-0 pt-0.5">Languages</span>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 sm:p-8 shadow-sm space-y-4">
        ...
        <div className="divide-y divide-slate-100">
          {documents.map((doc) => (
            <div key={doc.id} className="py-4 first:pt-2 last:pb-2 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
