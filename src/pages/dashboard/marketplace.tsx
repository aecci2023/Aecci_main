...
import { MapPin, ArrowRight, Briefcase, GraduationCap, Loader2, Search as SearchIcon, Globe, Users } from "lucide-react";
...
export default function MarketplacePage() {
  ...
  const partners = useMemo(() => {
    ...
    if (filterCountry) {
      list = list.filter((p: any) => p.expertiseCountries?.includes(filterCountry));
    }
    if (search.trim()) {
      ...
      list = list.filter((p: any) =>
        ...
        p.expertiseSectors?.some((s: string) => s.toLowerCase().includes(q)) ||
        p.expertiseCountries?.some((c: string) => c.toLowerCase().includes(q))
      );
    }
    return list;
          ...
          {!isLoading && (
            <div className="flex gap-4 shrink-0">
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground flex items-center gap-1"><Users className="size-3" /> Partners</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-emerald-600">{countries.length}</p>
                <p className="text-xs text-muted-foreground flex items-center gap-1"><Globe className="size-3" /> Countries</p>
              </div>
            </div>
          )}
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {countries.map((country) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => setFilterCountry(c => c === country ? "" : country)}
                  className={filterCountry === country ? "bg-primary/10 text-primary hover:bg-primary/20 border-none text-xs" : "text-xs"}
                >
                  {country}
                </Button>
              ))}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-12 border rounded-xl bg-muted/10 text-center space-y-2">
            ...
            <p className="text-muted-foreground text-sm">
              No verified partners found{filterCountry ? ` for ${filterCountry}` : ""}{search ? ` matching "${search}"` : ""}.
            </p>
            <Button variant="outline" size="sm" onClick={() => { setFilterCountry(""); setSearch(""); }}>
              Clear filters
            </Button>
          </div>
        ) : (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {partners.map((partner: any) => {
                ...
                const initials = userDetails.fullName
                  ...
                  : "PA";
                const tierClass = TIER_COLORS[partner.tier] || TIER_COLORS.Standard;
                  ...
                  >
                    <CardHeader className="pb-3 flex flex-row gap-4 items-start">
                      <Avatar className="h-14 w-14 rounded-xl border-2 border-background shadow-sm shrink-0">
                        {userDetails.profilePicture ? (
                          <img src={userDetails.profilePicture} alt={userDetails.fullName} className="size-full object-cover rounded-xl" />
                        ) : (
                      ...
                      <div className="space-y-1 flex-1 min-w-0">
                        <Badge className={`text-[10px] font-semibold px-2 border ${tierClass}`}>
                          {partner.tier} Expert
                        </Badge>
                      ...
                      <div className="flex items-start gap-1 text-[11px] text-muted-foreground">
                        ...
                        <span>
                          ...
                          <strong className="text-foreground/80">
                            ...
                            {partner.expertiseCountries?.length > 4 ? ` +${partner.expertiseCountries.length - 4} more` : ""}
                          </strong>
                        </span>
                      </div>

                      <p className="text-xs text-muted-foreground line-clamp-3 leading-relaxed">
                        {partner.bio || "This partner facilitates global trade compliance and buyer matchmaking."}
                      </p>
                    ...
                    <CardFooter className="border-t border-border/50 pt-3 pb-3 bg-muted/10 justify-between items-center text-xs">
                      <span className="text-muted-foreground text-[10px] flex items-center gap-1">
                        ...
                        {userDetails.languagesSpoken?.slice(0, 2).join(", ") || "English"}
                      </span>
