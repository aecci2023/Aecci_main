  ...
  const verificationColumns: ColumnDef<UserRow>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge className={verified ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : "bg-amber-500/10 text-amber-600 border-amber-500/20"} variant="outline">
    ...
    },
    {
      ...
      header: "Action",
      cell: ({ row }) => (
        <Button variant="outline" size="sm" onClick={() => handleReview(row.original)} className="gap-2">
  ...
  return (
    <Main fluid className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Review submitted KYC documents for newly registered business accounts.
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <DataTable columns={verificationColumns} data={users} searchKey="companyName" />
      )}
    </Main>
  );
}
