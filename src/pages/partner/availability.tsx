...
import { useGetPartnerProfileQuery, useSetupPartnerProfileMutation } from "@/store/api/adminApi";
...
export default function PartnerAvailabilityPage() {
  ...
  const [weeklySchedule, setWeeklySchedule] = useState<Record<string, ScheduleDay>>({
  ...
  useEffect(() => {
    if (partnerProfile?.availability?.weekly) {
      ...
      DAYS_OF_WEEK.forEach(day => {
        merged[day] = savedWeekly[day] || { enabled: false, start: "09:00", end: "17:00" };
      });
  ...
  const handleSave = async () => {
    ...
    for (const day of DAYS_OF_WEEK) {
      ...
      if (sched.enabled && sched.start >= sched.end) {
        toast.error(`Invalid times for ${day}. Start time must be before end time.`);
        return;
      }
    }
    ...
    return (
      <Main fluid className="flex justify-center p-10">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-muted-foreground animate-pulse text-sm">Loading availability...</p>
        </div>
      </Main>
    );
  }
  ...
  return (
    <Main fluid className="space-y-6 max-w-4xl mx-auto py-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-emerald-600 bg-clip-text text-transparent">Availability Settings</h1>
          <p className="text-muted-foreground mt-1">Define your weekly working hours and time slots for hosting Live Deal Room sessions.</p>
        </div>
        <Button onClick={handleSave} disabled={isSaving} className="w-full md:w-auto self-start">
          {isSaving ? (
      ...
      <Card className="shadow-md border-border">
        <CardHeader>
          ...
          <CardDescription>
            Configure your active availability status and times for each day of the week.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-3">
            {DAYS_OF_WEEK.map((day) => {
              const sched = weeklySchedule[day];
              return (
                <div key={day} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-muted/20 border rounded-lg gap-4 transition-colors hover:bg-muted/30">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <Label htmlFor={`day-${day}`} className="font-semibold text-base w-24 cursor-pointer">{day}</Label>
                  </div>
                  ...
                  {sched.enabled ? (
                    ...
                    <span className="text-sm text-muted-foreground italic sm:pr-8">Unavailable</span>
                  )}
                </div>
              );
            })}
