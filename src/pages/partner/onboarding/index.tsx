...
import { CheckCircle2, User, Globe, Calendar, ChevronRight, ChevronLeft, Loader2 } from "lucide-react";
import { toast } from "sonner";

const STEPS = [
  { title: "Personal Profile", description: "Tell clients about yourself", icon: User },
  { title: "Expertise & Focus", description: "Countries and sectors you specialise in", icon: Globe },
  { title: "Availability", description: "Set your weekly schedule", icon: Calendar },
];

const step1Schema = z.object({
  organization: z.string().min(2, "Organisation name is required").max(200),
  professionalTitle: z.string().min(2, "Professional title is required").max(150),
...
export default function PartnerOnboardingPage() {
  ...
  const handleStep3 = async (values: Step3) => {
    ...
    const payload = {
      ...
      expertiseCountries: s2Data.expertiseCountriesRaw.split(",").map((s) => s.trim()).filter(Boolean),
      expertiseSectors: s2Data.expertiseSectorsRaw.split(",").map((s) => s.trim()).filter(Boolean),
        ...
        <Card className="max-w-md w-full text-center">
          <CardContent className="pt-10 pb-8 space-y-4">
            ...
            <p className="text-muted-foreground text-sm">
              Your partner profile is set up. You can now start accepting client sessions and grow your practice through AECCI Global Deal Room.
            </p>
    ...
    <div className="min-h-screen bg-muted/20 p-6">
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <div className="text-center space-y-1">
          <h1 className="text-2xl font-bold tracking-tight">Welcome to AECCI Global Deal Room</h1>
          <p className="text-muted-foreground text-sm">Complete your partner profile to start working with clients</p>
        </div>
        ...
        {step === 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>This information is visible to clients when they browse partners</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form1}>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. Global Trade Advisors Pvt Ltd" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. Senior Trade Consultant" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
        ...
        {step === 1 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Clients filter partners by country and sector — be specific</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form2}>
                <form onSubmit={form2.handleSubmit(handleStep2)} className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. UAE, Saudi Arabia, Germany (comma-separated)" {...field} />
                        </FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g. Pharmaceuticals, Textiles, IT Services" {...field} />
                        </FormControl>
                        <p className="text-xs text-muted-foreground">Separate multiple sectors with commas</p>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Years of International Trade Experience</FormLabel>
                        <FormControl>
                          <select
                            {...field}
        ...
        {step === 2 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Let clients know when you are typically available for sessions</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form3}>
                <form onSubmit={form3.handleSubmit(handleStep3)} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {selectedDays.length === 0 ? "No days selected" : `${selectedDays.length} day(s) selected`}
                    </p>
                  </div>
