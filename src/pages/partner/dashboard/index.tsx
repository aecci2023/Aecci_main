...
export default function PartnerDashboard() {
  ...
  const [setupProfile, { isLoading: isSettingUp }] =
    useSetupPartnerProfileMutation();
  ...
  const { data: mySessionsData, refetch: refetchSessions } =
    useGetMySessionsQuery();
  ...
  const [submitSummary, { isLoading: isSubmittingSummary }] =
    useSubmitSessionSummaryMutation();
...
`.trim();
    ...
    try {
      ...
      console.error(err);
      toast.error(
        err?.data?.message || "Failed to submit post-session summary.",
      );
    }
  };
  ...
  const pastSessions = mySessions.filter(
    (s: any) => s.status === "completed" || s.status === "upcoming",
  );
  ...
  const needsSetup =
    partnerProfile && (!partnerProfile.bio || !partnerProfile.signedAgreement);

  if (needsSetup) {
    return (
      <Main fluid className="py-8 space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground text-sm">
            Please build your Partner Brief and availability settings to host
            Deal Room sessions.
          </p>
        </div>
            ...
            <form onSubmit={handleCompleteSetup} className="space-y-8">
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-lg border-b pb-2">
                  1. Partner Brief (Public Profile)
                </h3>
              ...
              <div className="space-y-4 pt-4 border-t">
                <h3 className="font-semibold text-lg flex items-center gap-2 border-b pb-2">
                  <Calendar className="size-5 text-primary" /> 2. Weekly
                  Availability Schedule
                </h3>
                <p className="text-xs text-muted-foreground">
                  Select the days you are available to host sessions and define
                  your active hours.
                </p>
                ...
                <div className="space-y-3">
                  {DAYS_OF_WEEK.map((day) => {
                    ...
                    return (
                      <div
                        key={day}
                        className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-muted/30 border rounded-lg gap-4"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <Label
                            htmlFor={`day-${day}`}
                            className="font-semibold text-sm w-24 cursor-pointer"
                          >
                            {day}
                          </Label>
                        </div>
                        ...
                        {sched.enabled ? (
                          ...
                          <span className="text-xs text-muted-foreground italic sm:pr-8">
                            Unavailable
                          </span>
                        )}
                      </div>
              ...
              <div className="space-y-4 pt-4 border-t">
                <h3 className="font-semibold text-lg flex items-center gap-2 border-b pb-2">
                  <FileText className="size-5 text-primary" /> 3. Digital
                  Agreement
                </h3>
                <div className="bg-muted p-4 rounded-md text-xs text-muted-foreground h-32 overflow-y-auto border">
                  <p className="font-semibold mb-1">
                    AECCI Partner Terms & Code of Conduct
                  </p>
                  <p>By signing this digital agreement, you commit to:</p>
                  <ul className="list-disc pl-5 mt-2 space-y-1">
                    <li>
                      Maintaining absolute confidentiality of all member
                      information, trade documents, and business briefs shared.
                    </li>
                    <li>
                      Providing verified, accurate trade compliance guidance,
                      import/export rules, and localized market insights.
                    </li>
                    <li>
                      Conducting live video sessions on-time with suitable
                      networking environments (high bandwidth).
                    </li>
                    <li>
                      Avoiding direct communication or business routing
                      off-platform with clients met via the platform.
                    </li>
                  </ul>
                </div>
                <div className="flex items-center gap-2.5 mt-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setSignedAgreement(checked === true)
                    }
                  />
                  <Label
                    htmlFor="agreement"
                  ...
                  >
                    I have read and agree to the AECCI Partner Code of Conduct.
                  </Label>
                </div>
              </div>
              ...
              <Button
                ...
                disabled={isSettingUp || !signedAgreement}
              >
  ...
  return (
    <Main fluid className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage assigned clients, set pricing, view schedule, and submit
            session summaries.
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Scheduled 1-on-1 bilateral deal rooms with exporters.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {upcomingSessions.length === 0 ? (
                  <p className="text-sm text-muted-foreground py-6 text-center border-2 border-dashed rounded-lg">
                  ...
                  upcomingSessions.map((session: any) => (
                    <div
                      key={session.id}
                      className="p-4 border rounded-lg bg-muted/20 flex flex-col gap-3"
                    >
            ...
            <Card className="shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Submit post-session summaries to trigger client Opportunity
                  Reports.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        onChange={(e) =>
                          handlePriceChange(user.id, e.target.value)
                        }
                        className="h-9"
                      />
                      <Button
            ...
            {users.length === 0 && (
              <p className="text-muted-foreground col-span-full text-sm">
                No pending clients assigned to you at the moment.
              </p>
            )}
          </div>
        </TabsContent>
      ...
      <Dialog
        open={!!summarySession}
        onOpenChange={(open) => !open && setSummarySession(null)}
      >
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Submit outcomes, actionable next steps, local contacts, and
          ...
          {summarySession && (
            <div className="space-y-4 py-3 text-xs leading-relaxed max-h-[60vh] overflow-y-auto pr-1">
              <div className="bg-muted/50 p-3 rounded-lg border">
                ...
                <p className="italic">
                  "
                  {summarySession.questionnaire ||
                    "No questionnaire details submitted."}
                  "
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="outcomes" className="text-xs font-semibold">
                  1. Session Outcomes & Advice Provided{" "}
                  <span className="text-red-500">*</span>
                </Label>
                <Textarea
                  id="outcomes"
              ...
              <div className="space-y-2">
                <Label htmlFor="nextSteps" className="text-xs font-semibold">
              ...
              <div className="space-y-2">
                <Label
                  htmlFor="localContacts"
                  className="text-xs font-semibold"
                >
                  3. Verified Local Contacts Promised (Optional)
                </Label>
                <Textarea
                  id="localContacts"
                  value={localContacts}
              ...
              <div className="space-y-2">
                <Label
                  ...
                  className="text-xs font-semibold"
                >
                  4. Recommended Paid Follow-Up Services (Optional)
                </Label>
                <Textarea
                  id="recommendations"
